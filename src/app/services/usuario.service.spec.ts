...
describe('UsuarioService', () => {
  ...
  beforeEach(() => {
    ...
    // obtengo el controlador que me permite interceptar las peticiones HTTP
  ...
  it('should be created', () => {
    ...
    // solo verifico que el servicio exista
  ...
  it('debería registrar un usuario usando POST', () => {
    ...
    // preparo la respuesta que simulará el backend
    const respuestaMock = {
      id: 1,
      ...usuarioMock,
      activo: true
    ...
    // valido que el método HTTP sea POST
  ...
  it('debería buscar un usuario por email usando GET', () => {
    ...
    service.buscarPorEmail(email).subscribe(usuario => {
      ...
      // verifico que el email retornado sea correcto
    ...
    // valido que se use GET
    expect(req.request.method).toBe('GET');

    // respondo con el usuario simulado
    req.flush(usuarioMock);
  });

  it('debería obtener la lista de usuarios usando GET', () => {

    // esta es la estructura típica HAL que viene del backend, se explñica en otro servcie
    const respuestaMock = {
      _embedded: {
        usuarioModelList: [
    ...
    service.getAll().subscribe(usuarios => {
      ...
      // verifico que el map haya funcionado
    ...
    // confirmo que se hizo un GET
    expect(req.request.method).toBe('GET');

    // envío la respuesta simulada
    req.flush(respuestaMock);
  });
